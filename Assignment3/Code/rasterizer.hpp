...
namespace rst
{
    ...
    inline Buffers operator|(Buffers a, Buffers b)//reload '|' and '&'
    {
    ...
    private:
        ...
        std::map<int, std::vector<Eigen::Vector3i>> ind_buf;//index is integer
        std::map<int, std::vector<Eigen::Vector3f>> col_buf;
        std::map<int, std::vector<Eigen::Vector3f>> nor_buf;

        std::optional<Texture> texture;//optional texture


        // load the fragment shader and the vertex shader in the form of Vector3f
        std::function<Eigen::Vector3f(fragment_shader_payload)> fragment_shader;
        std::function<Eigen::Vector3f(vertex_shader_payload)> vertex_shader;

        std::vector<Eigen::Vector3f> frame_buf;//store the color info in every pixel
        std::vector<float> depth_buf;
        int get_index(int x, int y);//index of the pixel
