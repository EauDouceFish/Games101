...
{
    //id start from 0, each id record a position
    auto id = get_next_id();//get the next position id
...
//the format to compute Barycentric
...
//Draw all the triangles to form the picture.
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    {
        Triangle newtri = *t;//Create new triangle, for every triangle make a trans
        ...
        Eigen::Vector4f v[] = {
                ...
                mvp * t->v[1],
                mvp * t->v[2]
        };//identify the position in 3D space
        ...
        //create basic color for every triangle
        newtri.setColor(0, 148,121.0,92.0);
        newtri.setColor(1, 148,121.0,92.0);
        newtri.setColor(2, 148,121.0,92.0);

        // Also pass view space vertice position
        rasterize_triangle(newtri, viewspace_pos);//This position is in 3D.
    }
}
...
}//Triangle in 2D, interpolate for (u,v).
    ...
    for(int x=min_x; x<max_x; x++)
    {
        for(int y=min_y; y<max_y; y++)
        {
            if(insideTriangle(x+0.5, y+0.5, t.v))
            {
                ...
                //    These have been finished in the last Homework.
